...
######################################################################################################
                                       #0.颜色矩阵修改
# 各种标签所对应的颜色
colormap = [[0, 0, 0], [128, 0, 0], [0, 128, 0], [128, 128, 0], [0, 0, 128], [128, 0, 128], [0, 128, 128], [128, 128, 128], [64, 0, 0]]
...
def train(offset, model, lr_update=False, show_img=False):
    ...
    optimizer = torch.optim.SGD(net.parameters(), lr=LR, weight_decay=1e-4,momentum=0.9)
    learning_rate = LR
    # 开始训练
    print("开始训练(〃'▽'〃)")
    ########################################################################################################
    ...
    for epoch in range(EPOCHES):
        ...
        path = "./checkpoint/res_segnet/%s_%s_epoch-%03d_loss-%.4f_loss(val)-%.4f_acc-%.4f_miou-%.4f_miou(val)-%.4f.pth" % \
        ...
        with open("log.txt", "a") as f:
            f.write("%s_%s_epoch-%03d_loss-%.4f_loss(val)-%.4f_acc-%.4f_miou-%.4f_miou(val)-%.4f"% \
               (model_name, datetime_str,epoch + offset, epoch_loss, val_loss, epoch_acc, epoch_miou, val_miou))

        #保存hist矩阵
        # Hist_path = "./pred_pic/%s_epoch-%03d_train_hist.png" % (datetime_str,epoch + offset)
        # tools.drawHist(hist, Hist_path)

        # 更新学习率
        if (lr_update == True):
            # 每20个epoch就将学习率降低10倍
            if (epoch_acc >= 0.90 and epoch_acc < 0.915):
                learning_rate = 5e-6
