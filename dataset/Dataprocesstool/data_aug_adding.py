...
def TestOneDir_label(root_path):
    save_path = root_path
    for a, b, c in os.walk(root_path):
        for file_i in c:
            file_i_path = os.path.join(a, file_i)
            print(file_i_path)
            img_i = Image.open(file_i_path)

            # img_scale = Scale(img_i,1.5)
            # cv2.imwrite(os.path.join(save_path, file_i[:-4] + "_scale.jpg"), img_scale)

            img_horizontal = img_i.transpose(Image.FLIP_LEFT_RIGHT)
            img_horizontal.save(os.path.join(save_path, file_i[:-4] + "_horizontal.png"))

            #
            img_vertical = img_i.transpose(Image.FLIP_TOP_BOTTOM)
            img_vertical.save(os.path.join(save_path, file_i[:-4] + "_vertical.png"))

            #
            ###########注意*********由于便签不能变颜色，所以标签不做变化******************#######
            # img_darker_labelnochange= img_i
            # img_darker_labelnochange.save(os.path.join(save_path, file_i[:-4] + "_darker.png"))

            ###########注意*********由于便签不能变颜色，所以标签不做变化******************#######
            # img_brighter_labelnochange = img_i
            # img_brighter_labelnochange.save(os.path.join(save_path, file_i[:-4] + "_brighter.png"))
            # img_rotate = img_i.rotate(90, expand=True)
            # img_rotate.save(os.path.join(save_path, file_i[:-4] + "_rotate90.png"))
...
if __name__ == "__main__":
    ...
    # TestOnePic()
    img_root_path = "../../Process_data_bigimagecrop/Processed_data_smallimage/Image"
    TestOneDir(img_root_path)
    label_root_path = "../../Process_data_bigimagecrop/Processed_data_smallimage/Label"
    TestOneDir_label(label_root_path)


    #../../Process_data_bigimagecrop/Processed_data_smallimage/Image
